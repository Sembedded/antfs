/*
 * logfile.h - Exports for $LogFile handling.
...
struct RECORD_PAGE_HEADER {
	/*  0*/
	enum NTFS_RECORD_TYPES magic;  /* Usually the magic is "RCRD". */
...
struct LOG_RECORD {
	...
	le32 transaction_id;
	enum LOG_RECORD_FLAGS log_record_flags;
 ...
 */
struct ATTR_OLD {    /* Format up to Win10 (44 bytes) */
	le64 unknown1;
	le64 unknown2;
	le64 inode;
	uint64_t lsn;
...
struct ATTR_NEW {    /* Format since Win10 (40 bytes) */
	le64 unknown1;
	le64 unknown2;
	le32 type;
	le32 unknown3;
...
extern bool ntfs_is_logfile_clean(struct ntfs_attr *log_na,
				  struct RESTART_PAGE_HEADER *rp);
extern int ntfs_empty_logfile(struct ntfs_attr *na);
