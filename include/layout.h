/*
 * layout.h - Ntfs on-disk layout structures.
...
/*  0	NTFS_RECORD; -- Unfolded here as gcc doesn't like unnamed structs. */
	enum NTFS_RECORD_TYPES magic;	/* Usually the magic is "FILE". */
...
enum ATTR_DEF_FLAGS {
	ATTR_DEF_INDEXABLE = const_cpu_to_le32(0x02),	/* Attribute can be
							   indexed. */
	ATTR_DEF_MULTIPLE = const_cpu_to_le32(0x04),
	    /* Attribute type can be present multiple times in the mft records
	     * of an inode. */
	ATTR_DEF_NOT_ZERO = const_cpu_to_le32(0x08),
	    /* Attribute value must contain at least one non-zero byte. */
	ATTR_DEF_INDEXED_UNIQUE = const_cpu_to_le32(0x10),
	    /* Attribute must be indexed and the attribute value must be unique
	     * for the attribute type in all of the mft records of an inode. */
	ATTR_DEF_NAMED_UNIQUE = const_cpu_to_le32(0x20),
	    /* Attribute must be named and the name must be unique for the
	     * attribute type in all of the mft records of an inode. */
	ATTR_DEF_RESIDENT = const_cpu_to_le32(0x40),	/* Attribute must be
							   resident. */
	ATTR_DEF_ALWAYS_LOG = const_cpu_to_le32(0x80),
	    /* Always log modifications to this attribute, regardless of whether
	     * it is resident or non-resident.  Without this, only log
...
/*hex ofs*/
	...
	/* 88*/
	enum COLLATION_RULES collation_rule;
			/* Default collation rule. */
	/* 8c*/
	enum ATTR_DEF_FLAGS flags;
...
enum ATTR_FLAGS {
	...
	ATTR_COMPRESSION_MASK = const_cpu_to_le16(0x00ff),
		/* Compression method mask. Also, first illegal value. */
...
enum RESIDENT_ATTR_FLAGS {
	RESIDENT_ATTR_IS_INDEXED = 0x01,
		/* Attribute is referenced in an index (has implications for
		 * deleting and modifying the attribute). */
} __attribute__ ((__packed__));
		...
		struct {
			...
			le16 value_offset;
				/* Byte offset of the attribute
				   ...
				   of 8-bytes. */
			/* 22 */
			enum RESIDENT_ATTR_FLAGS resident_flags;
				/* See above. */
			/* 23 */
			s8 reservedR;
		...
		struct {
			...
			uint64_t highest_vcn;
				 ...
				 * attribute value. - Usually there is only one
				   ...
				   zero length files. Can be 0 for "single
				   extent" attributes. */
			/* 32*/
			le16 mapping_pairs_offset;
				/* Byte offset from the
				   beginning of the structure to the mapping
				   pairs array which contains the mappings
				   between the vcns and the logical cluster
				   numbers (lcns). When creating, place this at
				   the end of this record header aligned to
				   8-byte boundary. */
			/* 34*/
			u8 compression_unit;
   ...
   be difficult to keep them up-to-date.*/
			...
			sle64 allocated_size;
				/* Byte size of disk space
				   ...
				   is a multiple of the cluster size. When a
				   file is compressed, this field is a multiple
				   of the compression block size
				   (2^compression_unit) and it represents the
				   logically allocated space rather than the
				   actual on disk usage. For this use the
				   compressed_size (see below). */
			/* 48*/
			sle64 data_size;
			...
			sle64 initialized_size;
				/* Byte size of initialized portion of the
...
/*Ofs*/
	...
	/* 32*/
	enum FILE_ATTR_FLAGS file_attributes;
...
/* 3c*/ union {
	...
	u8 file_name_length;
		/* Length of file name in
		   (Unicode) characters. */
	/* 41*/
	enum FILE_NAME_TYPE_FLAGS file_name_type;
...
struct OBJECT_ID_ATTR {
	struct GUID object_id;		/* Unique id assigned to the
				   file. */
	...
	union {
		struct {
			struct GUID birth_volume_id;
				/* Unique id of volume on which
				   the file was first created.*/
			struct GUID birth_object_id;
				/* Unique id of file when it was
				   first created. */
			struct GUID domain_id;	/* Reserved, zero. */
		} __attribute__ ((__packed__));
		u8 extended_info[48];
	} __attribute__ ((__packed__));
} __attribute__ ((__packed__));
...
struct SID {
	...
	union SID_IDENTIFIER_AUTHORITY identifier_authority;
 ...
 */
struct GENERIC_MAPPING {
	enum ACCESS_MASK generic_read;
	enum ACCESS_MASK generic_write;
	enum ACCESS_MASK generic_execute;
	enum ACCESS_MASK generic_all;
} __attribute__ ((__packed__));
...
struct ACCESS_ALLOWED_OBJECT_ACE {
	...
	/* Access mask associated with the ACE. */
	/*  8*/
	enum OBJECT_ACE_FLAGS object_flags;
...
struct SECURITY_DESCRIPTOR_RELATIVE {
	...
	u8 alignment;
	enum SECURITY_DESCRIPTOR_CONTROL control;
...
struct SECURITY_DESCRIPTOR {
	...
	/* Flags qualifying the type of
	   the descriptor as well as the following fields. */
	struct SID *owner;  /* Points to a SID representing an object's owner.
			If this is NULL, no owner SID is present in the
			descriptor. */
	struct SID *group;  /* Points to a SID representing an object's primary
			group. If this is NULL, no primary group SID is
			present in the descriptor. */
	struct ACL *sacl;  /* Points to a system ACL. Only valid, if
		       SE_SACL_PRESENT is set in the control field. If
		       SE_SACL_PRESENT is set but sacl is NULL, a NULL ACL
		       is specified. */
	struct ACL *dacl;  /* Points to a discretionary ACL. Only valid, if
		       SE_DACL_PRESENT is set in the control field. If
		       SE_DACL_PRESENT is set but dacl is NULL, a NULL ACL
		       (unconditionally granting access) is specified. */
} __attribute__ ((__packed__));
...
enum SECURITY_DESCRIPTOR_CONSTANTS {
	...
	SECURITY_DESCRIPTOR_MIN_LENGTH = sizeof(struct SECURITY_DESCRIPTOR),
};
...
struct SDS_ENTRY {
	...
	/* The self-relative security descriptor. */
} __attribute__ ((__packed__));
...
struct VOLUME_INFORMATION {
	...
	enum VOLUME_FLAGS flags;/* Bit array of VOLUME_* flags. */
} __attribute__ ((__packed__));
...
struct INDEX_HEADER {
	 ...
	 */
	/* 12*/
	enum INDEX_HEADER_FLAGS ih_flags;
...
struct INDEX_BLOCK {
/*  0	NTFS_RECORD; -- Unfolded here as gcc doesn't like unnamed structs. */
	enum NTFS_RECORD_TYPES magic;	/* Magic is "INDX". */
...
struct QUOTA_CONTROL_ENTRY {
	...
	enum QUOTA_FLAGS flags;  /* Flags describing this quota entry. */
	...
	struct SID sid;    /* The SID of the user/object associated with
...
enum INDEX_ENTRY_FLAGS {
	INDEX_ENTRY_NODE = const_cpu_to_le16(1),
		/* This entry contains a sub-node, i.e. a reference to an index
		   block in form of a virtual cluster number (see below). */
	INDEX_ENTRY_END = const_cpu_to_le16(2),
		/* This signifies the last entry in an index block. The index
		   entry does not represent a file but it can point to a
		   sub-node. */
...
/*  8*/ le16 length;
/* 10*/ le16 key_length;
/* 12*/ enum INDEX_ENTRY_FLAGS flags;
/* 14*/ le16 reserved;
/* sizeof() = 16 bytes */
} __attribute__ ((__packed__));
...
/*  0	INDEX_ENTRY_HEADER; -- Unfolded here as gcc dislikes unnamed structs. */
	...
	le16 key_length;
		/* Byte size of the key value, which is in the
		   ...
		   multiple of 8-bytes. */
	/* 12*/
	enum INDEX_ENTRY_FLAGS ie_flags;
...
/*	End of INDEX_ENTRY_HEADER */
	...
	union {
		...
		struct GUID object_id;	/* $O index in FILE_Extend/$ObjId: The
				   object_id of the mft record found in
				   the data part of the index. */
		struct REPARSE_INDEX_KEY reparse;	/* $R index in
						   FILE_Extend/$Reparse. */
		struct SID sid;	/* $O index in FILE_Extend/$Quota:
...
struct EA_ATTR {
	...
	enum EA_FLAGS flags;    /* Flags describing the EA. */
...
struct EFS_ATTR_HEADER {
	/*  0*/
	le32 length;  /* Length of EFS attribute in bytes. */
	le32 state;  /* Always 0? */
	...
	/* MD5 hash of DRFs? */
	/* 64*/
	le32 offset_to_ddf_array;
	/* Offset in bytes to the array of data
	   decryption fields (DDF), see below.  Zero if
	   no DDFs are present. */
	le32 offset_to_drf_array;
	/* Offset in bytes to the array of data
...
struct EFS_DF_HEADER {
	...
	/* Offset in bytes to the credential header. */
...
struct EFS_DF_CREDENTIAL_HEADER {
	...
	union {
		...
		struct {
			...
			/* Size in bytes of public key blob. */
		} __attribute__ ((__packed__));
		/* Certificate thumbprint. */
		struct {
			/* 12*/
			le32 cert_thumbprint_header_size;
			/* Size in bytes of the header of the
			 * certificate thumbprint. */
			/* 16*/ le32 cert_thumbprint_header_offset;
			/* Offset in
...
enum INTX_FILE_TYPES {
	INTX_SYMBOLIC_LINK = const_cpu_to_le64(0x014B4E4C78746E49ULL),
		/* "IntxLNK\1" */
	INTX_CHARACTER_DEVICE = const_cpu_to_le64(0x0052484378746E49ULL),
		/* "IntxCHR\0" */
	INTX_BLOCK_DEVICE = const_cpu_to_le64(0x004B4C4278746E49ULL),
		/* "IntxBLK\0" */
};

struct INTX_FILE {
	enum INTX_FILE_TYPES magic;	/* Intx file magic. */
	union {
